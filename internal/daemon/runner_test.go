...
func TestRunnerProcessOnceResultForceNotifiesHandledSnapshot(t *testing.T) {
	cfg := config.Default()
	cfg.Check.NotifyInitial = true
	snapshot := ipdetect.Snapshot{PublicIP: "203.0.113.4"}
	hash := snapshot.Hash()
	...
	result, err := runner.ProcessOnceResultWithOptions(context.Background(), ProcessOptions{ForceNotify: true})
	if err != nil {
		t.Fatalf("process result: %v", err)
	}
	if result.Changed {
...
func TestRunnerForceDoesNotOverrideInitialNotificationDisabled(t *testing.T) {
	cfg := config.Default()
	cfg.Check.NotifyInitial = false
	snapshot := ipdetect.Snapshot{PublicIP: "203.0.113.4"}
	notifier := &fakeNotifier{name: "bark"}
	result, err := (Runner{
		...
		Notifiers: []notify.Notifier{notifier},
		Now:       func() time.Time { return time.Unix(100, 0).UTC() },
	}).ProcessOnceResultWithOptions(context.Background(), ProcessOptions{ForceNotify: true})
	if err != nil {
		t.Fatalf("process result: %v", err)
	}
	if notifier.calls != 0 {
	...
	if !result.Changed {
		t.Fatalf("expected first snapshot to be recorded as changed")
	}
	if result.Notified {
		t.Fatalf("expected notified=false")
	}
...
func TestRunnerForceDoesNotOverridePermanentProviderFailure(t *testing.T) {
	cfg := config.Default()
	snapshot := ipdetect.Snapshot{PublicIP: "203.0.113.4"}
	hash := snapshot.Hash()
	notifier := &fakeNotifier{name: "pushover"}
	result, err := (Runner{
		Config:   cfg,
		Detector: &fakeDetector{snapshot: snapshot},
		Store: &memoryStore{state: state.State{
